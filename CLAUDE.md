...
- **Sesión de pruebas manuales de CJ (9 sep 2026)**: pila completa (`docker-compose.yml`) levantada a pedido para que CJ probara los 3 microservicios con sus propias manos vía navegador (`admin`/`changeme`), siguiendo `RUNBOOK.md`. Se recrearon los tópicos de Redpanda (sin volumen persistente, se pierden en cada `docker compose down`+`up` completo) y se confirmó que los datos de referencia (`nd.branch`/`nd.sales_agent`) seguían en el volumen de Postgres de sesiones anteriores. **Al cierre de esta conversación, la pila puede haber quedado corriendo o apagada según lo último que CJ haya hecho manualmente después de la última respuesta registrada acá — la sesión nueva debe verificar el estado real (`docker ps`) antes de asumir cualquiera de los dos, no inferirlo de este documento.**

## Pendientes explícitos (en este orden — no reordenar sin que CJ lo pida)

1. **Simuladores en los 3 formularios** (Customer, Inventory, Order) — mismo alcance simple que los de Andes: crean registros reales espaciados en el tiempo usando los mismos endpoints POST reales del formulario (nada se simula fuera del flujo real). El de Order tiene la misma restricción de diseño que el de Andes: consulta pares customer/inventory disponibles **en vivo** en cada ciclo (sin sistema de reservas); si un ciclo no encuentra un par disponible, lo salta sin error — esto es comportamiento esperado, no un bug a corregir. **CJ los prueba manualmente antes de seguir con el punto 2** — no avanzar al reverse proxy hasta que los simuladores estén aprobados.
2. **Reverse proxy (Caddy)** para resolver que el navegador pide credenciales HTTP Basic por separado en cada uno de los 3 microservicios — al vivir en puertos distintos (8001/8002/8003) son orígenes distintos para el navegador, así que autenticarse en uno no cubre a los otros dos. Un proxy único (Caddy, por simplicidad de configuración) al frente de los tres, con un solo dominio/puerto público, resuelve esto sin tocar el código de negocio de los tres servicios (coherente con la decisión de diseño #4: "si en algún despliegue real conviene un gateway, es un cambio de infraestructura, no de código").

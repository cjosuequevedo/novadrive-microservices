...
- **Databricks — YA NO comparte workspace con Andes** (cambio del 9 sep 2026, pendiente #4 completo): NovaDrive vive ahora en el workspace de un compañero de análisis de datos (`DATABRICKS_HOST`/`DATABRICKS_TOKEN`/`DATABRICKS_WAREHOUSE_ID` propios en `novadrive/.env`, ya no los de la cuenta personal usada hasta Fase 12). Andes sigue en el workspace anterior (`dbc-5becafae-e891.cloud.databricks.com`) — **no se tocó nada de Andes en esta migración**, solo el `.env` de NovaDrive (local y VPS). Catálogo `novadrive_catalog`, schema `bronze` únicamente, tabla `novadrive_catalog.bronze.events` — mismo nombre, workspace distinto. **El Bronze acumulado hasta Fase 13 (todo el de hoy, incluidas las pruebas del VPS) quedó en la cuenta personal anterior, sin migrar** — para consultarlo hay que volver a esas credenciales viejas, no aparece en el catálogo nuevo. Ver hallazgos técnicos #15/#16 para el detalle de por qué se migró (crédito agotado) y los dos intentos fallidos de `WAREHOUSE_ID` antes del correcto.
